    ...
    public class Tbl_tour_dates : BaseClass, IMigration
    {
        public void Remigration(MySQL_DB pMysql, PostgreSQL_DB pPostgres)
        {
            MySqlDataReader dataReader = pMysql.Select("select id, seasonid, eventid, stateid, cast(perfdivtype as char) as perfdivtype," +
                                                       ...
                                                       "hotel_notes, online_balance_payments_enabled, event_date_status, show_time, use_online_scoring, workshop_notes" +
                                                       " from tbl_tour_dates;");
            pMysql.Message = "tbl_tour_dates - extraction - START";
            while (dataReader.Read())
            {
                string state = (dataReader["stateid"] == "") ? "'"+dataReader["stateid"]+"'": "null";
                pPostgres.Insert("insert into tbl_tour_dates(id, season_id, event_id, state_id, perf_div_types_id, web, entrylimit, mybtf, hotel_id, venue_id, " +
                                     "start_date, end_date, cutoff_start_date, cutoff_end_date, room, notes, is_finals,  entry_limits, weather_link, hotel_alt, custom_routine_durations, " +
                                     "default_category_order, routine_dist, webcast_this_city, hotel_notes, online_balance_payments_enabled, event_date_status, show_time, use_online_scoring, city_id, workshop_notes ) " +
                                "values('" + dataReader["id"] + "','" + dataReader["seasonid"] + "','" + dataReader["eventid"] + "'," + state + ",'" + dataReader["perfdivtype"] + "'," +
                                     "'" + Get_json_web_info(dataReader[5].ToString(), dataReader[6].ToString(), dataReader[7].ToString(), dataReader[8].ToString(), dataReader[9].ToString(), 
                                     dataReader[10].ToString(), dataReader[11].ToString(), dataReader[12].ToString()) + "','" +
                                     "" + Get_json_entry_info(dataReader[13].ToString(), dataReader[14].ToString(), dataReader[15].ToString(), dataReader[16].ToString(), dataReader[17].ToString(), 
                                     dataReader[18].ToString()) + "','" +
                                     "" + Get_json_mybtf(dataReader[19].ToString(), dataReader[20].ToString(), dataReader[21].ToString()) + "'," +
                                     "" + GetHotelId(dataReader[22].ToString(), dataReader[23].ToString(), dataReader[24].ToString(), dataReader[25].ToString(), dataReader[26].ToString(), 
                                     dataReader[27].ToString(), dataReader[28].ToString(), pPostgres) + ", " +
                                     "" + GetVenueId(dataReader[29].ToString(), dataReader[30].ToString(), dataReader[31].ToString(), dataReader[32].ToString(), dataReader[33].ToString(), 
                                     dataReader[34].ToString(), dataReader[35].ToString(), pPostgres) + "," +
                                     "" + NVL(dataReader["start_date"].ToString()) + "," + NVL(dataReader["end_date"].ToString()) + "," +NVL(dataReader["cutoff_date"].ToString()) + "," +
                                 "" +NVL(dataReader["cutoff_date2"].ToString()) + "," + Get_room_rate(dataReader["room_rate"].ToString(), dataReader["roomreservations"].ToString()) + "," +
                                     "'" + dataReader["notes"].ToString().Replace("'", "''") + "','" + CheckBool(dataReader["is_finals"].ToString()) + "'," +
                                 "'" + dataReader["entry_limits"].ToString().Replace("'", "''") + "','" + dataReader["weather_link"].ToString().Replace("'", "''") + "'," +
                                     "'" + dataReader["hotel_alt"].ToString().Replace("'", "''") + "','" + dataReader["custom_routine_durations"].ToString().Replace("'", "''") + "'," +
                                 "'" + dataReader["default_category_order"].ToString().Replace("'", "''") + "','" + dataReader["routine_dist"] + "'," +
                                 ...
                                 "'" + CheckBool(dataReader["use_online_scoring"].ToString()) + "'," +
                                     "" + GetId("select id from tbl_cities where name like '" + dataReader["city"].ToString().Replace("'", "''") + "';", pPostgres) + "," +
                                 ""+NVL(dataReader["workshop_notes"].ToString().Replace("'","''")) + ");");

            }
            pPostgres.Insert("insert into tbl_tour_dates(id, season_id, event_id) values(3,0,6);");
            pPostgres.Insert("insert into tbl_tour_dates(id, season_id, event_id) values(0,0,6);");
        ...
        private string Get_json_web_info(string p_web_workshop, string p_web_competition, string p_web_playlist, string p_web_results, string p_web_city_webcast_banner, string p_web_no_competition, string p_web_videos, string p_web_personalpdfs)
        {
            ...
            web.workshop = Convert.ToInt32(p_web_workshop);
            web.competition = Convert.ToInt32(p_web_competition);
            web.playlist = Convert.ToInt32(p_web_playlist);
            web.results = Convert.ToInt32(p_web_results);
            web.city_webcast_banner = Convert.ToInt32(p_web_city_webcast_banner);
            web.no_competition = Convert.ToInt32(p_web_no_competition);
            web.videos = Convert.ToInt32(p_web_videos);
            web.personalpdfs = Convert.ToInt32(p_web_personalpdfs);
            return web.ToString();
        }
        ...
        private string Get_json_entry_info(string p_entrylimit_total, string p_entrylimit_solos, string p_entrylimit_duotrios, string p_entrylimit_groups, string p_entrylimit_onesolo, string p_entrylimit_sologroup)
        {
            ...
            entrylimit.total = Convert.ToInt32(p_entrylimit_total);
            entrylimit.solos = Convert.ToInt32(p_entrylimit_solos);
            entrylimit._duotrios = Convert.ToInt32(p_entrylimit_duotrios);
            entrylimit.groups = Convert.ToInt32(p_entrylimit_groups);
            entrylimit.onesolo = Convert.ToInt32(p_entrylimit_onesolo);
            entrylimit.sologroup = Convert.ToInt32(p_entrylimit_sologroup);
        ...
        private string Get_json_mybtf(string p_mybtfregenabled, string p_mybtf_no_competition, string p_mybtf_photovideo_active)
        {
            ...
            myftf.regenabled = Convert.ToInt32(p_mybtfregenabled);
            myftf.no_competition = Convert.ToInt32(p_mybtf_no_competition);
            myftf.photovideo_active = Convert.ToInt32(p_mybtf_photovideo_active);
            return myftf.ToString();
        }

        private string Get_room_rate(string pParameter, string pReservation)
        {
            if (pParameter != "" && pParameter?[0].ToString() == "$")
            {
                try
                {
                    ...
                    json_web_info.type = JArray.FromObject(test);
                    json_web_info.reservation = pReservation;
        ...
        private string GetHotelId(string p_hotel_name, string p_hotel_website, string p_hotel_address, string p_hotel_city, string p_hotel_stateid, string p_hotel_zip, string p_hotel_phone, PostgreSQL_DB pPostgres)
        {
            ...
            query = pPostgres.Select("select distinct id " +
                                   "from tbl_hotels where name like '" + p_hotel_name.Replace("'", "''") + "' and hotel_website like '%" + WebUtility.UrlDecode(p_hotel_website) + "%';");
            string pom;
            while (query.Read())
            {
            ...
            if (p_hotel_name != "")
            {
                string p_city_id = GetId("select id from tbl_cities where name like '" + p_hotel_city.Replace("'", "''") + "'", pPostgres);
                pPostgres.Insert("insert into tbl_addresses(state_id, address, city_id, zip) values('" + p_hotel_stateid + "','" + p_hotel_address + "'," + p_city_id + ",'" + p_hotel_zip + "');");
                string p_address_id = GetId("select max(id) from tbl_addresses", pPostgres);
                pPostgres.Insert("insert into tbl_hotels(name, hotel_website, address_id) values('" + p_hotel_name.Replace("'", "''") + "','" + WebUtility.UrlDecode(p_hotel_website) + "','" + p_address_id + "');");
                string p_hotel_id = GetId("select max(id) from tbl_hotels", pPostgres);
